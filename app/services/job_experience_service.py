...
class JobExperienceService:
    ...
    def describe(self, job: ProcessingJob) -> dict[str, object]:
        ...
        seconds_since_update = max(0, int((now - reference).total_seconds())) if reference else None
        stalled = job.status == JobStatus.RUNNING.value and seconds_since_update is not None and seconds_since_update > 90
        if stalled:
        ...
        return {"effective_status": effective_status, "status_label": status_label, "next_action": next_action, "blocked_reason": blocked_reason, "seconds_since_update": seconds_since_update, "stalled": stalled, "can_cancel": job.status in {JobStatus.PENDING.value, JobStatus.RETRYING.value}, "can_retry": job.status == JobStatus.FAILED.value}
