...
Nesta Sprint `BLOCKED` é um **estado efetivo de UX**, não um novo valor persistido no banco. Isso evita migration e evita duplicar a fonte da verdade. O job continua `PENDING`/`RETRYING`; se o Worker estiver indisponível, a API deriva `effective_status=BLOCKED` com motivo e próxima ação.
...
Os workflows foram atualizados na branch `integration/ux-roadmap` para que CI, Quality e Docker sejam executados também em pull requests desse ciclo. A Sprint 2 foi recriada sobre a branch de integração já corrigida para eliminar o conflito gerado durante o bootstrap dessa configuração.
